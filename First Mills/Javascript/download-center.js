document.addEventListener("DOMContentLoaded", () => {
  ...
  sectionHeaders.forEach((header) => {
    header.addEventListener("change", () => {
      const section = header.closest(".section-download");
  ...
  document.querySelectorAll(".sub-checkbox").forEach((checkbox) => {
    checkbox.addEventListener("change", () => {
      const section = checkbox.closest(".section-download");
  ...
  downloadAllButton.addEventListener("click", () => {
    ...
    if (bodyLang === "en") {
      downloadFiles(["pdfs/إنجليزي/التقرير كاملًا انجليزي.pdf"]);
    } else {
      downloadFiles(["pdfs/عربي/التقرير كاملًا.pdf"]);
    }
  });
